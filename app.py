...
def make_prediction(input_data):
    try:
        ...
        if not prediction_successful:
            raise Exception("Could not find correct feature order.")
        ...
        st.write("1. Are all .pkl files present?")
        st.write("2. Does input match training feature order?")
        st.write("3. Were encoders trained on full dataset?")
        try:
...
# 🚀 Run prediction every time form is submitted
if submitted:
